...
module.exports = app => {
  ...
  api.submitQuiz = (course, quizData) => {
    return new Promise(async (resolve, reject) => {
      try{
        ...
        if(quiz.questions.length !== quizData.answers.length){
  ...
  */ 
  ...
  api.getQuestionResponses = (course, quiz, questionIndex) => {
    return new Promise(async (resolve, reject) => {
      ...
      course = await api.getCourse(course);
      //get quiz
      quiz = await api.getQuiz(quiz);
      //Covert string to integer (questionIndex)
      questionIndex = parseInt(questionIndex);
      if(isNaN(questionIndex)){
        return reject('invalid questionIndex');
      }
      let reference = quiz.questions[questionIndex];
  ...
  */ 
  ...
  app.get('/api/courses/:course/panels/:panelName/score', (req, res) => {
    return respond(res, api.panelResponse(req.params.course, req.params.panelName));
  });
  
  api.panelResponse = (course, panelName) => {
    return new Promise(async (resolve, reject) => {
      try {
        course = await api.getCourse(course);
        panelResponse.find({}).exec((err, panels) => {
          if(err) {
            return reject(err);
          }
          return resolve(panels);
